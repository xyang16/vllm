...
def test_moe_align_block_size(
    ...
    actual_mum_blocks = cdiv(int(actual_num_tokens.item()), block_size)
    assert (actual_expert_ids[:actual_mum_blocks] >= 0).all() and (
        actual_expert_ids[:actual_mum_blocks] < num_experts
